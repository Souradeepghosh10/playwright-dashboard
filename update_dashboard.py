...
trend_file = "trend.json"import os
import json
from jinja2 import Template
...
def count_tests_in_suite(suite, seen_tests):
    passed = 0
    failed = 0

    # Count tests at this suite level
    ...
    for child_suite in suite.get("suites", []):
        p, f = count_tests_in_suite(child_suite, seen_tests)
        passed += p
        failed += f
...
for date_folder in sorted(os.listdir(reports_dir)):
    ...
    if os.path.exists(report_json_path) and os.path.exists(html_report_path):
        ...
        for suite in data.get("suites", []):
            p, f = count_tests_in_suite(suite, seen_tests)
            total_passed += p
            total_failed += f
